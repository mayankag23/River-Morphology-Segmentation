...
#     #     # Raw spectral bands should still match num_channels.
#     #     if num_channels is not None and spectral_bands is not None:
#     #         n_bands = (
#     #             len(spectral_bands)
#     #             if hasattr(spectral_bands, "__len__")
#     #             else None
#     #         )
...
#     #     # Model input may include additional derived feature bands.
#     #     if model_in_ch > 0 and num_channels is not None:
#     #         if model_in_ch < int(num_channels):
#     #             issues.append(
#     #                 f"model.in_channels={model_in_ch} is smaller than "
#     #                 f"num_channels={num_channels}. "
#     #                 "The model cannot consume fewer channels than exist "
#     #                 "in the raw spectral input."
#     #            )
...
class PipelineValidator:
    ...
    def _check_channels(config: Any) -> list[str]:
        ...
        spectral_bands = getattr(config, "spectral_bands", None)
        model          = getattr(config, "model", None)
        model_in_ch    = int(getattr(model, "in_channels", 0)) if model is not None else 0
